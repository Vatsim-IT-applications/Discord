...
const client = new bot_1.default({ owners: config_1.owners, token: config_1.token });
client.start();
...
client.on('messageReactionAdd', (reaction, user) => {
    if(reaction.emoji.name === 'ticketreact') { // If the emoji name is ticketreact, we will create the ticket channel.
        ...
        client.roles.remove(  newrole,'added Guest role to user after reading the server roles');
        client.role.add(  member, 'Done adding role')
